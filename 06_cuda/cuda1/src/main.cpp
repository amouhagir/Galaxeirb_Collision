...
void Load_1024particules(const char * filename, particule_t *p){
...
void Update_position(particule_t *p, vector_t *acc, int size)
{
	...
	for (i = 0; i < size; ++i)
	{
		p[i].x += (p[i].vx + acc[i].x*MASSFACTOR*DAMP)*DT;
		p[i].y += (p[i].vy + acc[i].y*MASSFACTOR*DAMP)*DT;
		p[i].z += (p[i].vz + acc[i].z*MASSFACTOR*DAMP)*DT;
	}
}
...
int main( int argc, char ** argv ) {
  ...
  //Loading 1024 particules from dubinski file
	Load_1024particules(FILENAME, part);
	...
	while ( !done ) {
		...
		CUDA_MEMCPY(acc, d_acc, sizeof(particule_t) * N, cudaMemcpyDeviceToHost);
		//Update positions in CPU
	  Update_position(part, acc, N);
